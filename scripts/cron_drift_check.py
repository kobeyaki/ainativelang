#!/usr/bin/env python3
"""# Shim: delegates to openclaw/bridge/cron_drift_check.py — do NOT edit logic here; edit in bridge/.

``scripts/`` is not a Python package; this file loads ``openclaw/bridge/_shim_delegate.py`` via
importlib and runs the real drift checker with the current ``sys.argv``.
"""
