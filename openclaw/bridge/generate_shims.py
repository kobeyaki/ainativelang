...
"""# Shim: delegates to openclaw/bridge/{target} — do NOT edit logic here; edit in bridge/.

``scripts/`` is not a Python package, so relative imports like ``from ..openclaw.bridge`` do not
apply. This shim loads ``openclaw/bridge/_shim_delegate.py`` via importlib and re-executes the
...
Implementation and flags are defined in openclaw/bridge/ only.
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Suggest or create scripts/ shims for openclaw/bridge/*.py. "
    ...
    print("=== AINL OpenClaw bridge — suggested scripts/ shims ===\n")
    ...
    print("- Prefer openclaw/bridge/ainl_bridge_main.py for CLI discovery when no scripts/ shim is required.")
