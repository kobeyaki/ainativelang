...
if [[ "${OPENCLAW_SKIP_NPM:-}" != "1" ]]; then
  if command -v npm >/dev/null 2>&1; then
    ...
    echo "    Install the OpenClaw CLI per https://github.com/openclaw/openclaw (e.g. npm install -g openclaw && openclaw onboard)." >&2
  fi
else
...
echo "==> Upgrading ainl-lang[benchmark,mcp] via python3 -m pip"
python3 -m pip install --upgrade 'ainl-lang[benchmark,mcp]'
