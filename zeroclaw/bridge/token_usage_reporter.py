#!/usr/bin/env python3
"""Summarize token/budget signals for ZeroClaw crons (``~/.zeroclaw/workspace/memory``)."""
...
def _memory_dir() -> Path:
    from zeroclaw.bridge.zeroclaw_memory_adapter import zeroclaw_memory_dir

    return zeroclaw_memory_dir()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Token usage / budget reporter for bridge + OpenClaw memory.")
    ap.add_argument("--dry-run", action="store_true", help="skip openclaw_memory append (stdout/JSON only)")
    ...
    mem = ZeroclawMemoryAdapter()
    mem.call("append_today", [body], {"dry_run": dry})

    if args.notify and not dry:
        sys.stderr.write("[notify] stub: wire QueuePut in supervisor.ainl or OPENCLAW notify hook.\n")
